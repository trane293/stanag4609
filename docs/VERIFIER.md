...
`video.level.picture_size` checks the uncropped coded dimensions against the
AVC `MaxFS`/dimension limits or HEVC `MaxLumaPs`/dimension limits for every
observed SPS. `video.level.sample_rate` additionally applies AVC `MaxMBPS` or
HEVC `MaxLumaSr` whenever sequence timing is present. An absent timing signal is
reported as unverifiable rather than inferred from transport timestamps.
