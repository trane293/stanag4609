...
ST1201_DEPRECATED = {
    f"ST 1201.1-{index:02d}" for index in range(1, 12)
}
ST1303_ACTIVE = {
    "ST 1303-02",
    "ST 1303-03",
...
def test_requirement_inventory_names_the_exact_normative_populations() -> None:
    ...
    st1201 = documents["MISB-ST-1201.5"]
    assert set(st1201["active_requirements"]) == ST1201_ACTIVE
    assert len(st1201["active_requirements"]) == len(ST1201_ACTIVE) == 7
    assert {
        entry["id"] for entry in st1201["inactive_requirements"]
    } == ST1201_DEPRECATED
    assert len(st1201["inactive_requirements"]) == len(ST1201_DEPRECATED) == 11
    assert {
        entry["status"] for entry in st1201["inactive_requirements"]
    } == {"deprecated"}

    st1303 = documents["MISB-ST-1303.2"]
    assert set(st1303["active_requirements"]) == ST1303_ACTIVE
    assert len(st1303["active_requirements"]) == len(ST1303_ACTIVE) == 10
    assert {
        entry["id"] for entry in st1303["inactive_requirements"]
    } == ST1303_DEPRECATED
    assert len(st1303["inactive_requirements"]) == len(ST1303_DEPRECATED) == 11
    assert {
        entry["status"] for entry in st1303["inactive_requirements"]
    } == {"deprecated"}

    st1402 = documents["MISB-ST-1402.2"]
    assert set(st1402["active_requirements"]) == ST1402_ACTIVE
    assert len(st1402["active_requirements"]) == len(ST1402_ACTIVE) == 22
    assert {
        entry["id"] for entry in st1402["inactive_requirements"]
    } == ST1402_DEPRECATED
    assert len(st1402["inactive_requirements"]) == len(ST1402_DEPRECATED) == 4
    assert {
        entry["status"] for entry in st1402["inactive_requirements"]
    } == {"deprecated"}
